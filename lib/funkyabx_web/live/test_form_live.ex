    ...
    ~H"""
      <.form class="mb-2" let={f} for={@changeset} phx-change="validate" phx-submit={@action}>
            ...
            <fieldset class="form-group mb-3">
              ...
              <div class="px-3 pt-2 pb-3 rounded-3" style="background-color: #583247;">
                ...
                <%= unless @test.type == :listening do %>
                  <div class="mb-3">
                    <label for="" class="form-label">Private results page</label>
                    <div class="input-group mb-3">
                      <%= if @current_user do %>
        ...
        <fieldset>
          ...
          <% else %>
            <div class="alert alert-info alert-thin"><i class="bi bi-info-circle"></i>&nbsp;&nbsp;Supported formats: wav, mp3, aac, ogg, flac ... <a href="https://en.wikipedia.org/wiki/HTML5_audio#Supported_audio_coding_formats" target="_blank">(details)</a>. Wav files are converted to flac. No normalization or gain correction is applied.</div>
          <% end %>
              ...
              [{original_filename, filename}] =
                for entry <- entries do
                  consume_uploaded_entry(socket, entry, fn %{path: path} ->
                    ...
                    Files.upload(path, Path.join([socket.assigns.test.id, filename_dest]))
